...
execute 'add crowd database' do
  ...
  not_if do
    `#{my_exe} --execute "show databases;"`.split("\n").include?(node['crowd']['mysql']['dbname'])
  end
end
...
execute 'add crowd user to database' do
  ...
  not_if do
    `#{my_exe} --execute "select user from mysql.user"`.split("\n").include?(
      node['crowd']['mysql']['username']
    ) &&
      `#{my_exe} --execute "select user from db where db = '#{node['crowd']['mysql']['dbname']}' and user = '#{node['crowd']['mysql']['username']}'"`.split("\n").include?(
        node['crowd']['mysql']['username']
      )
  end
end
...
execute 'unpack connectorj' do
  ...
  action :nothing
  subscribes :run, "remote_file[#{jcon_local}]", :immediately
end
...
ruby_block 'install connectorj for crowd' do
  ...
  only_if do
    ...
    end.detect { |e| e == false }.nil?  # Ensures all exists? checks return true!
  end
end
